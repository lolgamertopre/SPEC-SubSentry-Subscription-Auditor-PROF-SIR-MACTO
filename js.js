...
// Convert yearly cost to monthly equivalent
const toMo = (cost, cycle) => cycle === 'yearly' ? cost / 12 : cost;

// Format number as currency string: 15 → "$15.00"
...
// Pick a color palette based on first letter of name
const iconStyle = name => PALETTES[name.charCodeAt(0) % PALETTES.length];

// Get up to 2 initials from service name
...
function addSub() {
  ...
  // Validate: name required, cost must be a positive number
  if (!name || isNaN(cost) || cost <= 0) {
    toast('Please fill in all fields correctly.', 'error');
    return $('svc-name').focus();
  }

  // Warn if duplicate name exists (but still allow adding)
...
function startEdit(id) {
  ...
  // Commit edit on blur or Enter key
  function commitEdit() {
...
// ── RENDER ───────────────────────────────────────
// Redraws the entire UI from the current subs array
...
function render() {
  ...
  if (!subs.length) {
    ...
    // Build one card per visible subscription
  ...
  // Update the 3 dashboard numbers
...
// Press Enter in name or cost field to submit
['svc-name', 'svc-cost'].forEach(id =>
  $(id).addEventListener('keydown', e => e.key === 'Enter' && addSub())
);

// Highlight name field yellow while typing if duplicate detected
...
// Initial render — loads persisted data from localStorage on page load
render();